    ...
    test.step("GIVEN: Retrieve address", async () => {
      ...
      await test.step("WHEN: Retrieve address amount", async () => {
        ...
        await test.step("AND: Retrieve address amount field", async () => {
          let quantity = addressAmount.map(
            (addressAmountDto) => addressAmountDto.quantity
          );
          let unit = addressAmount.map(
            (addressAmountDto) => addressAmountDto.unit
          );

          await test.step("THEN: Verify address field amount is not null", async () => {
            Assertions.assertNotNull(
              quantity,
              "address amount quantity should not be null"
            );
            Assertions.assertNotNull(
              unit,
              "address amount unit should not be null"
            );
          });
        });
      });
