...
test.describe("@regression @governance-rule", () => {
  ...
  test("Check get gov ratification result for new consitution", async ({}) => {
    test.step("GIVEN: Retrieve get gov ratification result for new consitution", async () => {
      let newConstitution = await (
        await yaciGovernanceRuleService()
      ).getRatificationResultForNewConstitutionAction();

      await test.step("THEN:gov ratification result for new consitution should not be null", () => {
        Assertions.assertNotNull(
          newConstitution,
          "gov ratification result for new consitution should not be null."
        );
      });
    });
  });
