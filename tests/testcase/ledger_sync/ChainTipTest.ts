    ...
    test.step('STEP 1: Retrieve chain tip', async () => {
      ...
      await test.step('STEP 3: Retrieve chain tip again', async () => {
        ...
        await test.step('VP: Compare chain tip again', () => {
          Assertions.assertEqual(chainTipLSAfterWait, chainTipKoiosAfterWait, 'Chain tips should be equal after waiting.');
        });

        await test.step('VP: Assert the chain tip sync after a period of time', () => {
          Assertions.assertNotEqual(String(chainTipLS), String(chainTipKoiosAfterWait), 'Chain tips should not be equal after waiting.');
          Assertions.assertNotEqual(String(chainTipKoios), String(chainTipLSAfterWait), 'Chain tips should not be equal after waiting.');
        });
      });
    });
  });
