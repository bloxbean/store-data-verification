    ...
    test.step('STEP 1: Get random addresses', async () => {
      ...
      test.step('STEP 2: Retrieve address balance comparison maps', async () => {
        ...
        test.step('VP: Compare balances', () => {
          for (const [addressBalanceComparisonKey, addressBalanceComparison] of addressBalanceComparisonMapLS.entries()) {
          ...
          test.step('VP:Assert map sizes are equal', () => {
            Assertions.assertEqual(Object.keys(addressBalanceComparisonMapKoios).length, String(Object.keys(addressBalanceComparisonMapLS).length), 'Balance should be equal.');
          });
        });
      });
    });
