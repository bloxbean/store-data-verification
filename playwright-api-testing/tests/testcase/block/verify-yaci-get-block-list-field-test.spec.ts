  ...
  test("Check the logic of process a new block", async ({}) => {
    test.step("GIVEN: Retrieve block list", async () => {
      ...
      await test.step("WHEN: Retrieve block list information", async () => {
        ...
        let slot_leader = blockListYaci.map((blockListYaciDto) => blockListYaciDto.slot_leader);
        let size = blockListYaci.map((blockListYaciDto) => blockListYaciDto.size);
        let tx_count = blockListYaci.map((blockListYaciDto) => blockListYaciDto.tx_count);
        let issuer_vkey = blockListYaci.map((blockListYaciDto) => blockListYaciDto.issuer_vkey);
