          ...
          await test.step("AND: Retrieve block_hash from both pool ", () => {
            ...
            test.step("THEN: pool_id_bench32 , tx_hash and block_hash for the pool with same epoch_no should be equal ", () => {
              Assertions.assertEqual(
                poolIdBench32Yaci,
                poolIdBench32Koios,
                "pool_id_bench32 should be the same."
              );
              Assertions.assertEqual(
                txHashYaci,
                txHashKoios,
                "tx_hash should be the same."
              );
              Assertions.assertEqual(
                blockHashYaci,
                blockHashKoios,
                "block_hash should be the same."
              );
            });
          });
        });
