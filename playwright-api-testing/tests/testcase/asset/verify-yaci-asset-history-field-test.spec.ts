  ...
  test("Compare the asset history of Koios and Yaci Store", async ({}) => {
    test.step("GIVEN: get detail unit information", async () => {
      let detailTransaction = await (
        await yaciService()
      ).getDetailTransaction(TxHashes.TX_HASHES_1);

      await test.step("WHEN: get min and max value of all field", async () => {
        let block_number = detailTransaction.map(
          (detailTransactionDto) => detailTransactionDto.block_number
        );
        let block_time = detailTransaction.map(
          (detailTransactionDto) => detailTransactionDto.block_time
        );
        let output_index = detailTransaction.map(
          (detailTransactionDto) => detailTransactionDto.output_index
        );
        let slot = detailTransaction.map(
          (detailTransactionDto) => detailTransactionDto.slot
        );
        let epoch = detailTransaction.map(
          (detailTransactionDto) => detailTransactionDto.epoch
        );
        let lovelace_amount = detailTransaction.map(
          (detailTransactionDto) => detailTransactionDto.lovelace_amount
        );
        let amount = detailTransaction.map(
          (detailTransactionDto) => detailTransactionDto.amount
        );
