  ...
  test("Check the field of epoch parameter", async ({}) => {
    test.step("GIVEN: Retrieve epoch parameter", async () => {
      ...
      await test.step("WHEN: Get epoch parameter field", async () => {
        let min_fee_a = epochParameter.map(
          (epochParameterDto) => epochParameterDto.min_fee_a
        );
        let min_fee_b = epochParameter.map(
          (epochParameterDto) => epochParameterDto.min_fee_b
        );
        let max_block_size = epochParameter.map(
          (epochParameterDto) => epochParameterDto.max_block_size
        );
        let max_block_header_size = epochParameter.map(
          (epochParameterDto) => epochParameterDto.max_block_header_size
        );
        let key_deposit = epochParameter.map(
          (epochParameterDto) => epochParameterDto.key_deposit
        );
        let pool_deposit = epochParameter.map(
          (epochParameterDto) => epochParameterDto.pool_deposit
        );
        let a0 = epochParameter.map(
          (epochParameterDto) => epochParameterDto.a0
        );
        let rho = epochParameter.map(
          (epochParameterDto) => epochParameterDto.rho
        );
        let tau = epochParameter.map(
        ...
        let extra_entropy = epochParameter.map(
          (epochParameterDto) => epochParameterDto.extra_entropy
        );
        let protocol_major_ver = epochParameter.map(
          (epochParameterDto) => epochParameterDto.protocol_major_ver
        );
        let protocol_minor_ver = epochParameter.map(
          (epochParameterDto) => epochParameterDto.protocol_minor_ver
        );
        let min_utxo = epochParameter.map(
          (epochParameterDto) => epochParameterDto.min_utxo
        );
        let nonce = epochParameter.map(
          (epochParameterDto) => epochParameterDto.nonce
        );
        let cost_models = epochParameter.map(
          (epochParameterDto) => epochParameterDto.cost_models
        );
        let price_mem = epochParameter.map(
          (epochParameterDto) => epochParameterDto.price_mem
        );
        let price_step = epochParameter.map(
          (epochParameterDto) => epochParameterDto.price_step
        );
        let max_tx_ex_mem = epochParameter.map(
          (epochParameterDto) => epochParameterDto.max_tx_ex_mem
        );
        let max_tx_ex_steps = epochParameter.map(
          (epochParameterDto) => epochParameterDto.max_tx_ex_steps
        );
        let max_val_size = epochParameter.map(
          (epochParameterDto) => epochParameterDto.max_val_size
        );
        let collateral_percent = epochParameter.map(
          (epochParameterDto) => epochParameterDto.collateral_percent
        );
        let max_collateral_inputs = epochParameter.map(
          (epochParameterDto) => epochParameterDto.max_collateral_inputs
        );
        let coins_per_utxo_size = epochParameter.map(
          (epochParameterDto) => epochParameterDto.coins_per_utxo_size
        );
        let pvt_motion_no_confidence = epochParameter.map(
          (epochParameterDto) => epochParameterDto.pvt_motion_no_confidence
        );
        let pvt_committee_normal = epochParameter.map(
        ...
        let committee_min_size = epochParameter.map(
